...
- Toasts in Komponententests: `sonner` mocken (`toast` als `vi.fn()` mit `success/error/warning`) und die `action.onClick` direkt auslösen – der echte `Toaster` braucht Pointer-Capture, das jsdom nicht hat. Seed-Zeilen (Budget) hängen an der echten Uhr: in Tests, die eine bestimmte Woche erwarten, fremde Budgetzeilen im Setup entfernen.
